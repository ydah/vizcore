...
Mappings inside a `custom_shape` block are applied to each primitive generated by
that custom shape:
...
shape methods. The first implementation is static: `ctx.time` and `ctx.audio`
are available with default values, while per-frame dynamic custom shape
re-expansion remains future work.
